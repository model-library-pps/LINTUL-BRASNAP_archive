...
*     November 1 2022 Herman Berghuijs: the original code required Ångström coefficients
*     to be negative in the weather file, otherwise the radiation date were either read as
*     sunshine hours or, in case of Ångström coefficient with values outside a range, an error
*     was thrown. In the current code, I forced cRAD_TYPE equal to 1 such that the radiation data
*     that are read are always in kJ m-2 d-1.
      cRAD_TYPE = 1;
